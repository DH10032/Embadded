...
class DL(Dataset):
    ...
    def T(self):
       ...
       '''
        ...
        HandWriting_tensor = [transform(Image.open(file).resize((new_width, new_height))).to(DEVICE) for file in random.sample(HandWriting, self.num_img)]
        Printed_tensor = [transform(Image.open(file).resize((new_width, new_height))).to(DEVICE) for file in random.sample(Printed, self.num_img)]
...
with torch.no_grad():
    ...
    for x, y in val_loader:
        image = x.to(DEVICE)      # tensor를 image에 저장
        label= y.to(DEVICE)

        outputs = model.forward(image)
        _, predicted = torch.max(outputs.data, 1)           # _, 생략의 의미
        total += label.size(0)

...
<details>
  ...
  a) 로봇이 물건 찾는 작동 방식
    1. 사진을 서버에 올림
    2. 서버가 이미지를 뿌림
    3. 이미지에 맞는 물건을 찾는게 목표 - SLAM
    4. 로봇이 지나가는 구간에 찾고자 하는 물건이 없으면, 지도에 표시 - SLAM
    5. 의심되는 물건은 사진을 찍어서 서버에 보내고 위치를 표시 - 카메라 인식
    6. 로봇이 찾아볼수 없는 부분은 지도에 알려주기 - 상자 같은 서랍 구분이 필요(딥러닝)
  b) 주차/월별 계획
    3월 5주차 ~ 4월 2주차
      이미지 처리/딥러닝•머신러닝
    4월 3주차 ~ 5월 1주차
      OpenCV: 이미지 처리, SLAM에 이용
      SLAM: 센서 신호 처리(움직임 추정, 장애물 회피), 그래프 최적화
      *특이 사항: 4월 말~5월 초에 대회 공지가 올라올 예정이므로 공지 체크할 것.
    5월 2주차 ~ 5월 5주차

...
model = torchvision.models.vgg16(pretrained=True).to(DEVICE) # PyTorch 라이브러리의 torchvision.models 모듈. VGG16 모델은 합성곱 신경망(CNN) 아키텍처이며, 이미지 분류 작업에서 자주 사용.
...
req_features = []
output = dummy_img.clone().to(DEVICE) # PyTorch에서 이미지를 복제하고, 해당 복제본을 지정된 장치(예: CPU 또는 GPU)로 이동시키는 코드
...
transform = transforms.Compose([transforms.ToTensor()]) # 이미지를 텐서로 변환하는 코드는 PyTorch의 torchvision.transforms 모듈을 이용한 전처리 과정
imgTensor = transform(img).to(DEVICE) # transform(img): 이미지에 정의된 변환을 적용(여기서는 tensor로), 후 .to(DEVICE)는 cpu 또는 cuda로 변환된 텐서를 옮김
imgTensor = imgTensor.unsqueeze(0)
output_map = faster_rcnn_feature_extractor(imgTensor) # 이미지 텐서로부터 특징 맵(feature map)을 얻는 코드
...
for i in range(5):
    fig.add_subplot(1, 5, figNo) # Matplotlib을 사용하여 하나의 그림(fig)에 여러 개의 서브플롯(subplot)을 추가하는 코드. fig 객체에 1행 5열의 서브플롯 중 figNo번째 서브플롯을 추가.
...
feature_size = 800 // 16
ctr_x = np.arange(16, (feature_size + 1) * 16, 16) # NumPy를 사용하여 일정한 간격으로 배열을 생성하는 코드
...
for i in range(ctr.shape[0]):
    cv2.circle(img_clone2, (ctr_int[i][0], ctr_int[i][1]), # cv2.circle(img, center, radius, color, thickness): 이미지 img 위에 중심이 center인 원을 그림.
...
for i in range(11025, 11034):
    ...
    cv2.rectangle(img_clone, (x1, y1), (x2, y2), color=(255, 0, 0), #cv2.rectangle(img, pt1, pt2, color, thickness): 이미지 img 위에 두 꼭지점 pt1과 pt2로 정의된 직사각형을 그림.
                 thickness=3)
...
img_clone3 = np.copy(img) # NumPy를 사용하여 이미지 img의 복사본을 만드는 코드. 이를 통해 원본 이미지를 변경하지 않고도 복사본을 사용하여 다양한 작업을 수행
img_clone4 = cv2.copyMakeBorder(img_clone3,400,400,400,400,cv2.BORDER_CONSTANT, value=(255, 255, 255)) # cv2.copyMakeBorder(src, top, bottom, left, right, borderType, value): 원본 이미지 src의 주위에 테두리를 추가
...
conv1.weight.data.normal_(0, 0.01) # PyTorch에서 특정 레이어의 가중치를 정규 분포를 사용하여 초기화하는 코드. conv1이라는 이름의 합성곱(convolution) 레이어의 가중치를 평균 0, 표준 편차 0.01인 정규 분포로 초기화.
conv1.bias.data.zero_() #  PyTorch에서 특정 레이어의 편향(bias) 값을 0으로 초기화하는 코드. 이 코드는 conv1이라는 이름의 합성곱(convolution) 레이어의 편향값을 모두 0으로 설정
...
neg_index = np.where((max_iou < neg_iou_thresh_hi) & # NumPy를 사용하여 특정 조건을 만족하는 인덱스를 찾는 코드. 객체 검출 및 이미지 분할 작업에서 자주 사용, 네거티브 샘플(negative samples)을 선택하는 데 유용
...
n_sample = roi_cls_loc.shape[0] # roi_cls_loc 텐서의 첫 번째 차원의 크기를 n_sample 변수에 저장하는 코드. 주로 텐서의 샘플 수를 가져올 때 사용.
roi_loc = roi_cls_loc.view(n_sample, -1, 4) # PyTorch 텐서의 형태를 변경하는 코드. 이 코드는 roi_cls_loc 텐서를 (n_sample, -1, 4) 형태로 다시 배열.
# print(roi_loc.shape)
...
rpn_loss = rpn_loss.to(DEVICE) # PyTorch에서 텐서 또는 변수 rpn_loss를 특정 장치(예: CPU 또는 cuda)로 이동시키는 코드

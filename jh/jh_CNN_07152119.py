...
num_epoch = 20              # 전체 데이터셋을 학습 하는 횟수
...
class CNN(nn.Module):                   # nn.module class 상속
    def __init__(self):
        ...
        self.layer = nn.Sequential(     # 각 모듈을 순차적으로 실행한다.
            ...
            nn.PReLU(),
            # Parametric ReLU(PReLU)는 Leaky ReLU의 한 변형으로, 음수 입력에 대해 고정된 작은 기울기 대신 학습 가능한 매개변수를 사용
            # f(x) = x if x > 0, ax if x ≤ 0로, 여기서 a는 학습을 통해 조정되는 매개변수
